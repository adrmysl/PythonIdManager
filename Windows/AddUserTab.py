...
from Services.EmployeeService import addNewEmployee
...
elementList = {'name', 'secondName', 'surname', 'position', 'endContract', 'dateOfBirth'}
...
def myDef():
    ...
    endContractCol = [[sg.Text("Data końca umowy"), sg.I(size=(20, 10), key='endContract', default_text='1.01.1900'),
                       sg.CalendarButton('', image_data=calendar_icon, format='%d.%m.%Y', target='endContract', key='calBtn1')]]
    ...
    layout = [[sg.Button(image_data=convert_file_to_base64('calendar.png'), image_subsample=4)],
              ...
              [sg.Frame('',
                        [[sg.Text("Data urodzin"), sg.I(size=(20, 10), key='dateOfBirth', default_text='1.01.2022'),
                          sg.CalendarButton("", image_data=sg.red_x, format='%d.%m.%Y', target='dateOfBirth')]])],
    ...
    while True:
        ...
        if event == 'createBtn':
            if checkInput(values, window['infoBar']):
                isOk = addNewEmployee(getEmployee(values))
        if event == 'eraseBtn':
            cleanInput()

            pr, isOk = executeSQLCommand("""SELECT * from Employee""")
            for i in pr:
                print(i)
