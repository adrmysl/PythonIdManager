import base64
import pathlib
import datetime

import PySimpleGUI as sg

from Models.Employee import Employee

calendar_icon = b'iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8' \
                b' '
elementList = {'name', 'secondName', 'surname', 'position', 'endContract', 'dateOfBirth'}
my_new_theme = {'BACKGROUND': '#FFFFFF',
                'TEXT': 'black',
                'INPUT': 'white',
                'TEXT_INPUT': 'black',
                'SCROLL': 'white',
                'BUTTON': ('black', 'white'),
                'PROGRESS': ('#FFFFFF', '#FFFFFF'),
                'BORDER': 1,
                'SLIDER_DEPTH': 0,
                'PROGRESS_DEPTH': 0}

sg.theme_add_new('my_custom_theme', my_new_theme)
sg.theme('my_custom_theme')


def checkInput(newEmployee, infoBar):
    infoBar.Update('')
    for value in newEmployee:
        if value in elementList and value != 'secondName':
            if value == 'dateOfBirth' or value == 'endContract':
                try:
                    datetime.datetime.strptime(newEmployee[value], '%d.%M.%Y')
                except ValueError:
                    print('Bledna data dla', value)
            if newEmployee[value] == '':
                infoBar.Update('Wprowadź wszystkie dane!')



def convert_file_to_base64(filename):
    try:
        fullPath = pathlib.Path().resolve() / 'icon'
        contents = open(fullPath / filename, 'rb').read()
        encoded = base64.b64encode(contents)
        return encoded
    except Exception as error:
        sg.popup_error('Cancelled - An error occurred', error)


def getEmployee(data):
    Employee(data)


def myDef():
    nameCol = [[sg.T("Imię"), sg.In(key='name')]]
    secondNameCol = [[sg.Text("Drugie imię"), sg.Input(key='secondName')]]
    surnameCol = [[sg.Text("Nazwisko"), sg.Input(key='surname')]]
    positionCol = [[sg.Text("Stanowisko"), sg.Input(key='position')]]
    endContractCol = [[sg.Text("Data końca umowy"), sg.I(size=(20, 10), key='endContract'),
                       sg.CalendarButton('', image_data=calendar_icon, format='%d.%M.%Y')]]

    layout = [[sg.Button(image_data=convert_file_to_base64('calendar.png'), image_subsample=4)],
              [sg.Text("Wprowadź dane:", border_width=5)],
              [sg.Frame('', nameCol)],
              [sg.Frame('', secondNameCol)],
              [sg.Frame('', surnameCol)],
              [sg.Frame('',
                        [[sg.Text("Data urodzin"), sg.I(size=(20, 10), key='dateOfBirth'),
                          sg.CalendarButton("", image_data=sg.red_x, format='%d.%M.%Y')]])],
              [sg.Frame('', positionCol)],
              [sg.Frame('', endContractCol)],
              [sg.Button("Utwórz pracownika", key="createBtn", pad=(0, 0, 0, 10)),
               sg.Button('Usuń dane', key="eraseBtn")],
              [sg.Text("", key="infoBar")]]
    window = sg.Window('IdManager v1.0', layout, default_element_size=(24, 1), auto_size_text=False, size=(580, 460))

    def cleanInput():
        for element in elementList:
            window[element].Update('')

    while True:
        event, values = window.read()
        if event in (None, 'Exit'):
            break
        if event == 'createBtn':
            checkInput(values, window['infoBar'])
        if event == 'eraseBtn':
            cleanInput()

    window.close()


myDef()
